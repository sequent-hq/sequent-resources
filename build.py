...
def video_card(v, brand):
    ...
    watch=f'<a class="button featured-watch" href="{esc(v["src"])}" data-video="{esc(v["src"])}" data-download="{esc(v["download"])}" data-title="{esc(v["title"])}" data-poster="{esc(v["image"])}">{icon("play")}Watch film</a>' if featured else ''
    return f'''<article class="video-card{' featured-video' if featured else ''}" data-brand-label="{esc(brand)}"><a class="video-cover" href="{esc(v['src'])}" data-video="{esc(v['src'])}" data-download="{esc(v['download'])}" data-title="{esc(v['title'])}" data-poster="{esc(v['image'])}" aria-label="Play {esc(v['title'])}"><span class="image-fallback">{icon('film')}<span>Video preview</span></span><img src="{esc(v['image'])}" alt="{esc(v['title'])}" loading="lazy"><span class="play-circle">{icon('play')}</span><span class="resolution">{esc(v['resolution'])}</span></a><div class="video-body"><span class="video-brand-label">{esc(brand)}{' · Featured film' if featured else ''}</span><h4>{esc(v['title'])}</h4><p>{esc(v['description'])}</p>{watch}<div class="video-actions">{external(v['download'],'Open collection','text-link')}<button class="icon-button jsonly copy-link" data-copy="{esc(v['download'])}" aria-label="Copy link to {esc(v['title'])}">{icon('copy')}</button></div></div></article>'''
